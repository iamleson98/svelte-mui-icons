<script lang="ts">
  export let size: number = 20;
  export let color: string = 'currentColor';
  export let viewBox: string = '0 0 24 24';
</script>
<svg xmlns="http://www.w3.org/2000/svg" enable-background="new 0 0 20 20" height={size} {viewBox} width={size}><rect fill={color} height={size} width={size}/><path d="M17.07,17.07L2.93,2.93L1.87,3.99L3,5.12V17h11.88l1.13,1.13L17.07,17.07z M4.5,15.5V6.62l8.88,8.88H4.5z M14,7V4.5h1.5 v8.88l1.5,1.5V3h-5.07c-0.22-0.86-1-1.5-1.93-1.5C9.07,1.5,8.29,2.14,8.07,3H5.12l4,4H14z M10,3c0.41,0,0.75,0.34,0.75,0.75 c0,0.41-0.34,0.75-0.75,0.75S9.25,4.16,9.25,3.75C9.25,3.34,9.59,3,10,3z"/></svg>