<script lang="ts">
  export let size: number = 20;
  export let color: string = 'currentColor';
  export let viewBox: string = '0 0 24 24';
</script>
<svg xmlns="http://www.w3.org/2000/svg" enable-background="new 0 0 20 20" height={size} {viewBox} width={size}><rect fill={color} height={size} width={size}/><path d="M17,5.5v6h-1.5V6.26l-4-2.91l-4,2.91V7.5H6v-2l5.5-4L17,5.5z M18,16v0.5l-6.5,2L6,16.82V18H1V9h6l5.59,2 c0.85,0.3,1.41,1.1,1.41,2h1C16.66,13,18,14.34,18,16z M4.5,10.5h-2v6h2v-0.14V10.5z M16.39,15.43c-0.23-0.54-0.76-0.93-1.39-0.93 H9.74l-2-0.8l0.53-1.4l1.87,0.7h2.36c0-0.26-0.17-0.5-0.42-0.59L6.74,10.5H6v4.75l5.5,1.68L16.39,15.43z M12.62,6h-0.75v0.75h0.75V6 z M11.12,6h-0.75v0.75h0.75V6z M12.62,7.5h-0.75v0.75h0.75V7.5z M11.12,7.5h-0.75v0.75h0.75V7.5z"/></svg>