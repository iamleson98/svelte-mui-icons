<script lang="ts">
  export let size: number = 20;
  export let color: string = 'currentColor';
  export let viewBox: string = '0 0 24 24';
</script>
<svg xmlns="http://www.w3.org/2000/svg" enable-background="new 0 0 20 20" height={size} {viewBox} width={size}><g><rect fill={color} height={size} width={size}/></g><g><path d="M15.66,11H14V6.25C14,5.56,13.44,5,12.75,5h0c-0.69,0-1.25,0.56-1.25,1.25V15L9.38,14.3c-0.54-0.18-1.13-0.04-1.54,0.36 L7.5,15l3.56,3.56c0.28,0.28,0.66,0.44,1.06,0.44h5.27c0.73,0,1.36-0.53,1.48-1.25l0.61-3.65c0.11-0.65-0.22-1.29-0.81-1.59 L15.66,11z M5,1v1h2.29L2,7.3V5H1v4h4V8H2.71L8,2.71V5h1V1H5z"/></g></svg>